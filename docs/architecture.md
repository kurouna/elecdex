...
**操作**: `Ctrl+Shift+Z`（keybindings.ts の `pane.zoom`。Web ペインのページからも main 経由で届く）、モジュール枠の ⤢ ボタン（× の隣）、シェル／タブグループはタブストリップの ⤢、暗幕のクリック、Escape。Escape はコードとして表現できない（Ctrl/Alt かファンクションキーが要る）ので Workspace が直接見る。ダイアログが開いている間はダイアログのもの。
