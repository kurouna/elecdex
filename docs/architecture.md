...
- 色はテーマの意味トークン: 賛成 `--ok`、反対 `--danger`、棄権 `--info`、書いている間 `--accent`。無効票のユニットは
  `--text-muted`（消灯した色）— 当初は `--warn` だったが、黄色は評議会のほかの色と合わないので外した（利用者判断
  2026-09-21）。理由は文字で示す（プレートの `NO CARRIER` など、下のカードの詳細）。決議の帯の DEADLOCK などと
  コンソールの警告行は `--warn` のまま。
...
- **3 枚のプレートは同じ面積**（利用者の指摘 2026-09-21。上の 1 枚が下の 2 枚より 2 割近く小さかった: 1253 対 1536）。
  上は高さが低いぶん幅を広げ（29–71% → 26–74%）、下の 2 枚は外側の辺を内へ寄せる（2–42% → 4.5–42%）。コアに
  向いた辺はどれも動かさないので、コアの位置・スポーク・パケットの口はそのまま。`plateArea` の単体テストが
  面積をそろえておく。上のプレートが左へ広がったぶん、コンソールの幅は 27cqw → 25cqw。
