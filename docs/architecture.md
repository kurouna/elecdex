...
| 問題 | 内容 | 対応予定 |
|---|---|---|
| ターミナルの画面状態の再生 | ペインが別のノードへ移って再マウントされる（例: 単独のターミナルで Ctrl+Shift+T を押しタブグループ化）と、表示中の履歴が崩れる。再接続時に PTY の生バイト列を再生しているが、Windows の ConPTY はカーソル位置指定で画面を描き直すため、再生しても元の画面にならない | `@xterm/addon-serialize` で画面状態を保存・復元し、main 側の再生と重複しないよう attach に再生の有無を渡す |
