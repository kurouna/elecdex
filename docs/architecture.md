...
| プラグイン仕様（v0.0.5） | widget プラグインのみ。`<userData>/plugins/*.ts` を main が sucrase で変換（実行はしない）、renderer のペインごとの常駐 blob Worker で動かす。描画はブロック宣言型（`Block[]` をホストが Svelte で描き、色は tone のみ）。通信は宣言・同意したホストへの GET だけを main が代行。置いただけでは無効で、有効化時に権限への同意を取り、権限が広がったら取り直す。詳細は docs/plugins.md | 現状ペインの追加はレイアウト側では疎結合（registry に1件足すだけ）だが、Svelte コンポーネントはサンドボックス外で `window.elecdex` に届くためプラグインに渡せない。旧設計の「呼ぶたびに Worker を作る」「Canvas プリミティブ」「通信なし」では常時更新・表や一覧・外部 API を読む用途に向かなかった。ブロック型はサンドボックスと CSP を緩めずにテーマ追従と eDEX の見た目を保てる。command プラグインは端末への書き込みの入口設計が未決のため後続版 |
