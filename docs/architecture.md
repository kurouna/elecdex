...
- **審議中に動き続けるものは 10fps の共有ループで段階送り**（2026-09-21、利用者が採用）。彗星・縁の光・TX と RX の
  パケット・床は CSS アニメーションをやめ、`lib/frame-loop.ts` の `onFrame` が渡すフレームの時刻から位置を決める
  （`light.ts` の `lap` / `packetProgress`、破線は `stroke-dashoffset` 属性、床は `--floor-at`）。着地したパケットは
  ループが片付ける（`inFlight`。`animationend` は使わない）。窓が隠れればループごと止まる。床は「止まった位置で
  止まる」ために計算済みの transform を読む必要がなくなった（位相は自分で持っている）。
  **実測の切り分け**（同じ測り方、2 席が RX、毎秒 10 断片）: そのまま 70%、床だけ止めて 74%、光の SVG だけ消して
  59%、両方で 51%、さらにプレートの `fill` のトランジションを切って 17% — 足し算にならない。どれか 1 つでも
  動いていればステージ全体が毎フレーム合成され、その費用がほぼ全部だった。プレートの塗りは拍（250ms）ごとに
  変わり、240ms のトランジションが付いていたので、事実上終わらないアニメーションだった（審議中は
  `transition: none`。拍は段で変わるもの）。**段階送りにした後は 28〜29%**（GPU 15% + Tab 12%）、待機中 0.5%。
  一瞬で終わるもの（票の輪郭・閃光・判・スポークの明滅・点灯・フェード）は CSS のまま。
...
| ELEC の審議中の費用（v0.0.12 の後、2026-09-21） | 詳細は §5.8。審議中に動き続けるもの（彗星・縁の光・パケット・床）を CSS アニメーションから 10fps の共有ループの段階送りに移し、拍で変わるプレートの塗りのトランジションを審議中は切った。1 コア比で 70% → 28〜29%。切り分けの実測で、費用は個々の演出ではなく「何か 1 つでも毎フレーム動いていること」（ステージ全体の 60fps 合成）だと分かったので、光だけでなく全部を移した。 |
