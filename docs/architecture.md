...
- 上段の **stage**: 3 枚のプレートを三角形に置き（UNIT-2 が上、UNIT-1 が左下、UNIT-3 が右下）、中央に六角形の
  ...
  ゲージを隠し、名前と票だけにする。上の隅には RULE・ROUND と ELAPSED / TOOK・VOTES・TOKENS の読み出し
  （狭いと隠す）。ステージはペインの 64%（最大 48rem）まで伸び、下の声明が 7rem を切る前に縮む。
...
- **光の演出**（2026-09-21、利用者の依頼「TRON っぽい SF 感。このペインは CPU・GPU・メモリを多少使ってよい」）。
  ...
  待機中は静止）、(2) コアの周りの破線の軌道と逆回りの弧、六角形の中を回るレーダーの扇（どれも `rotate`）、
  (3) 問い合わせ中のユニットのプレートの縁を 2 つの光が回る（RX になると速く）、(4) スポーク上のパケット —
  TX は コア→ユニット、RX は ユニット→コアの向き、(5) 輪を回る 2 つの彗星（プレートの下の層なので隙間にだけ見える）。
  一度きり: 票が入るとプレートの輪郭をその色で一周描いて消える、決議が出るとその色の光が 3 本のスポークを
  コアへ流れ込み、コアから衝撃波の輪が 2 つ広がる（見ているペインで審議中→決着を見たときだけ）。
...
| ELEC の光の演出（v0.0.12 の後、2026-09-21） | 詳細は §5.8「光の演出」。審議中だけ動く床・軌道・レーダー・縁の光・パケット・彗星と、票と決議の一度きりの演出。WebGL は使わず（GPU コンテキストの予算）、SVG の破線と transform だけ。実測で審議中 5.6%（1 コア比）、待機中 0%。 |
