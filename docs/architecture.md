...
| ペインの角のボタンをタブグループにも（v0.0.13 の後、2026-09-22） | 右上の ⤢ と ×（ホバーで出る）を `layout/PaneCorner.svelte` にまとめ、モジュール枠・単独のシェル・タブグループのすべてが同じ角に持つ。タブグループの × はグループごと閉じる（`layout.close` がタブノードの id も受け、グループ全体が 1 枚として電源オフしてから消える。前面に出ているグループはそのまま留めて電源オフ）。タブごとの × は残す（背面のタブをクリック 1 回で閉じられる唯一の手段）。タブストリップ右端の ⤢ はやめた | 閉じると前面表示の場所がペインの種類で違い、タブグループ・シェルでは右上に何もなく、ストリップの ⤢ は常時表示の平行四辺形で見た目も違っていた（ユーザー指摘 2026-09-22: ほかのペインと揃える。タブの × はブラウザ流に残す、右上はグループ全体の × と ⤢）。ラベルは表示中のタブのもので、⤢ の有無も表示中のタブのウィジェットで決める（§5.5 と同じ） |
