...
- ペインはログの切れ目に `NOT SENT · n ABOVE` の線を引く（警告色、理由と設定場所は title）。モデルが
  ...
  各回答の読み出しに既にある）。
- 要約（compact）は次の段階。Anthropic のサーバー側 compaction（beta）は、専用ブロックを履歴に保持して
  送り返す必要があり「text だけが履歴」の設計と合わないので使わない。
...
| AI チャットのコンテキストの窓（未リリース、2026-09-21） | 長い会話を全文送り続けていたのを、プロバイダごとの窓（`contextTokens`。未設定ならローカルのアドレスは 8192、ホスト型は 0 = 全文）に合わせて main が送る分だけ削る。正本とエクスポートは全文のまま。窓の 3/4 を超えたら 1/2 まで一度に切り、位置を会話に保存して次に超えるまで動かさない。見積もりは文字種別の概算（ASCII 4 文字 = 1、他は 1 文字 = 1）を、プロバイダの実測で上方向にだけ補正。ログの切れ目に `NOT SENT · n ABOVE`。詳細は §5.7 | ローカルの小さいモデルが会話の途中から崩れる（サーバーが先頭をシステムプロンプトごと黙って捨てる）ため。利用者の案（文字数の予算・グローバル設定・毎ターンの trim・直近 N ターン保護・`[CONTEXT]` の user メッセージ）から変えた点と理由: 文字数 → トークン概算（日本語で 4 倍ずれる）、グローバル → プロバイダごと（8k と 1M は 1 つの値にならない）、毎ターン → まとめて切って位置を保存（KV / prompt キャッシュを毎ターン捨てない）、直近 N ターン保護 → なし（予算を破れば元の問題に戻る）。既定値 8192 とホスト型 0 は利用者の判断（2026-09-21）。要約は次の段階 |
