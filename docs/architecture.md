...
| 既定レイアウトの再設計 | eDEX-UI の配置に合わせるのをやめ、ペインの役割で配置する。左「このマシン」: 時計（高さ 7% に縮小）・システム・CPU・メモリ・上位プロセス。中央「作業」: シェル3タブと、その下にファイルブラウザ（シェルの cwd に追従し入力もするため隣接させる）とランチャーを半分ずつ。右「外の世界」: ネットワーク状態・地球儀・通信量・相場・天気。列幅 20/58/22%、右列の見出しは PANEL / WORLD | 時計は1行の表示なのに列の 12% を占めていた。ファイルの格子は中央の全幅を必要としないため、半分をランチャー（作業の起点）に充てた。相場と天気は外部の情報なのでネットワークと同じ右にまとめた。1920×1080 で相場8銘柄のうち7銘柄半、天気の1週間分が収まり、1366×768 でも各ペインが読める大きさを保つことをスクリーンショットで確認。相場の行はスパークラインの canvas（既定の高さ 150px）が行の高さを押し広げていたため、canvas をフローから外した |
