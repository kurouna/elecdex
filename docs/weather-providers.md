...
状態: **設計レビュー待ち**（実装前）。対象: `src/shared/weather.ts`・`src/main/weather/`・`WeatherWidget.svelte`。
...
1. **米国の既定ソース**: 自動選択で NWS にするか（降水確率と英文の予報がある）、MET にするか（全世界で見た目が揃う）。依頼の「デフォルトは MET Norway」に合わせるなら、既定の地点ニューヨークでも MET を使い、NWS は設定で選ぶ形になる。
2. **都市一覧の規模**: 人口 50 万以上 + 首都（約 1,200 件）で足りるか、15,000 人以上（約 3 万件・約 2MB）まで入れるか。
3. **気温の単位**: 華氏の切り替えを入れるか。
