...
没有 ⤢，计算器、计时器、混音器、磁盘和进程列表则以面板形式出现在中间，而不是铺满整个窗口。
...
- **剪贴板** *(unreleased)* — 在 desk 预设中，也可以从选择器（"clipboard history"）添加。按从新到旧
  列出窗格显示在屏幕上时你复制的内容，并注明大小和多久以前。链接、路径、数字和颜色分别标为 URL、PATH、
  NUM 和 CLR（带色块）；带格式复制的文本（HTML，或 Word 和写字板复制的 RTF）标为 RICH；不带格式的普通
  文本没有标签。点击某一项（或按 Enter）即可连同复制时的格式放回剪贴板，再粘贴到任何地方；↑ ↓
  ...
  插件也无法访问。**MASK** 隐藏内容，**CLEAR**（按两次）清空列表。仅限文本：剪贴板上的图片和文件不会保留。
