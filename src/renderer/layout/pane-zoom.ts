...
/** The box a zoomed pane fills: `fraction` of the workspace, centred in it. */
export function zoomBox(area: Frame, fraction = ZOOM_FRACTION): Frame | null {
  ...
  const insetX = (width(area) - width(area) * fraction) / 2
  const insetY = (height(area) - height(area) * fraction) / 2
  return {
    top: area.top + insetY,
